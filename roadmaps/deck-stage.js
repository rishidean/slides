...
(() => {
  ...
  const stylesheet = `
    ...
    .btn.reset, .btn.notes, .btn.present {
      font-size: 11px;
      font-weight: 500;
    ...
    .btn.reset .kbd, .btn.notes .kbd, .btn.present .kbd {
      display: inline-flex;
      align-items: center;
      justify-content: center;
      min-width: 16px;
    ...
    .notes-panel .notes-body.notes-empty { color: rgba(255,255,255,0.4); font-style: italic; }
    ...
    @media print {
      ...
      .overlay, .rail, .rail-resize, .ctxmenu, .confirm-backdrop, .notes-panel { display: none !important; }
    }
  `;
  ...
  class DeckStage extends HTMLElement {
    ...
    _render() {
      ...
      overlay.innerHTML = `
        ...
        <span class="divider"></span>
        <button class="btn notes" type="button" aria-label="Toggle speaker notes" title="Speaker notes (N)">Notes<span class="kbd">N</span></button>
        <button class="btn present" type="button" aria-label="Enter presentation mode" title="Present fullscreen (P) — Esc exits">Present<span class="kbd">P</span></button>
      `;

      overlay.querySelector('.prev').addEventListener('click', () => this._advance(-1, 'click'));
      overlay.querySelector('.next').addEventListener('click', () => this._advance(1, 'click'));
      overlay.querySelector('.reset').addEventListener('click', () => this._go(0, 'click'));
      overlay.querySelector('.notes').addEventListener('click', () => this._toggleNotes());
      ...
      notesPanel.className = 'notes-panel export-hidden';
      notesPanel.setAttribute('data-omelette-chrome', '');
      notesPanel.innerHTML = `
        <div class="notes-label"><span>Speaker notes</span><span class="notes-count"></span></div>
        <div class="notes-body"></div>
      `;
      this._notesPanel = notesPanel;
      this._notesBody = notesPanel.querySelector('.notes-body');
      this._notesCount = notesPanel.querySelector('.notes-count');
      try { this._notesVisible = localStorage.getItem('deck-stage.notesVisible') === '1'; } catch (err) {}

      this._root.append(style, rail, resize, stage, overlay, menu, confirm, notesPanel);
      this._canvas = canvas;
      this._stage = stage;
      this._slot = slot;
    ...
    _fit() {
      ...
      if (this.hasAttribute('noscale')) {
        ...
        if (this._notesPanel) this._notesPanel.style.marginLeft = '0';
        return;
      }
      ...
      if (this._notesPanel) this._notesPanel.style.marginLeft = (rw / 2) + 'px';
    ...
    _toggleNotes() {
      ...
      try { localStorage.setItem('deck-stage.notesVisible', this._notesVisible ? '1' : '0'); } catch (e) {}
      this._syncNotes();
    }

    _syncNotes() {
      if (!this._notesPanel) return;
      const txt = (this._notes && this._notes[this._index]) || '';
      this._notesBody.textContent = txt || 'No speaker notes for this slide.';
      this._notesBody.classList.toggle('notes-empty', !txt);
      if (this._notesCount) this._notesCount.textContent = (this._index + 1) + ' / ' + (this._slides.length || 1);
      if (this._notesVisible) this._notesPanel.setAttribute('data-open', '');
      else this._notesPanel.removeAttribute('data-open');
    }

    /** Presentation mode: browser fullscreen on the document, chrome
     *  suppressed via the existing _presenting flag. Esc exits natively;
     *  _onFsChange restores normal mode on any fullscreen exit. */
    _togglePresent() {
